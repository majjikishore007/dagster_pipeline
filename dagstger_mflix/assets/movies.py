...
def user_engagement(snowflake: SnowflakeResource) -> None:
    """
    Generate a CSV file with the number of comments per movie
    """
...
def top_movies_by_month(context, snowflake: SnowflakeResource) -> None:
    """
    Generate a CSV file with the top movies by month based on IMDB rating
    """
    ...
    query = f"""
        ...
        order by movies.imdb__rating desc
        """
    with snowflake.get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(query)
